...
//print'<pre>';print_r($mentorships);die;
	$this->table->set_heading('Title','Image','Category Name','Modules','Status','Action');
	foreach($mentorships as $mentorship){
		...
		if($mentorship->status  == 1){
				$statu ='<div class="btn btn-info">Active</div>';
		}else{
				$statu ='<div class="btn btn-danger">Pending</div>'	;
		}
		
    $modules=get_modules($mentorship->id);
		
		$links = anchor(base_url().'mentor-dashboard/mymentorship-view/'.$mentorship->id,' ',array('title'=>'View mentorship','class'=>'fa fa-eye fa-lg nav_icon'));
		$links .= ' | '.anchor(base_url().'mentor-dashboard/mymentorship-edit/'.$mentorship->id,' ',array('title'=>'Edit mentorship','class'=>'fa fa-pencil fa-lg nav_icon'));
		$links .= ' | '. anchor('mentor-dashboard/mymentorship-remove/'.$mentorship->id,' ',array('title'=>'remove mentorship','class'=>'fa fa-trash fa-lg nav_icon','onclick'=>"return ConfirmDialog()"));
		$this->table->add_row($mentorship->title,$image,$mentorship->name,$modules,$statu,$links);
